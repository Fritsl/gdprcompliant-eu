...
**111 of 112 done.** Phase 0, phase 1 and phase 2 are complete, phase 3 has begun, DNS
...
call; the verifier rate as the dashboard number with its alarm), and the rule corpus (45
rules across the Union, Denmark and Germany, six where the two countries genuinely part,
golden duty sets for three synthetic companies per jurisdiction), and the second
jurisdiction end to end (German for every guide, remedy, rule and web message; a German
case bound, rendered and checked for any Danish trace; no country branch in detector or UI
code), and the canary (a public corpus with an owner and a pace, nightly snapshots that
...
alone, the case page printed to A4), and referral and benchmarks (the ask only on a fix
confirmation, a first-party code and nothing else, the benchmark from the canary corpus
honest about its size), and processing agreement discovery and analysis (three findings
with three remedies, sixteen elements as content, the breach window, the sub-processor
objection and the transfer annex read in code, ten labelled agreements through the
verifier gate), and sub-processor recursion (lists read in three languages, a
breadth-first walk with enforced depth and node caps, cycles kept as edges, every edge
carrying its document and date, one request per host per interval with robots.txt
honoured, the chain on the graph as engages edges), and the processing agreement and
sub-processor page generators (one clause per element of the Article 28 table, each
traceable to its provision; the page from the chain, row by row with the list and the day;
a lawyer’s-review notice inside both), and crawl etiquette (a named user agent pointing at
a published behaviour page rendered from the content the scanner reads, one central
limiter and robots reader on every browser context, no form submission, no consent gate,
and a deep scan only for a claimed case or a recorded public-interest decision), and the
supply-chain map (one model drawn as SVG, PNG and PDF in greys only, sixty nodes over
three levels without a box touching another, every node with its jurisdiction and a link
to the evidence that placed it), and the behavioural eval sets (twenty policies, twenty
agreements, twenty planner states and sixty verifier claims, each with its reasoning;
thresholds declared once and enforced in CI; results folded into a committed history; the
self-hosted model the only one measured), and the case-grounded advisor (an answer in
three parts kept apart: the answer, what the case says with a pointer per fact, what the
law says quoted verbatim for the case's jurisdiction; a refusal names the settling
question; every answer recorded on the timeline and carried in the report), and verbatim
law (a quotation is fetched by identifier and never typed by a model, compared character
for character before it is shown, refused when shortened, dotted or annotated, and dated
with the text it speaks from), and the advisor's limits (a not-legal-advice notice on the
page and every export; it answers before it refers and a referral-first answer is refused;
case facts travel fenced as untrusted data and a verdict word in the answer fails the
call; refusals are a registry set of their own; answers are tenant-scoped and deleted with
the case), and dive points (any finding, quoted article or answer opens a conversation
scoped to it: the fragment stripped, capped at 300 and fenced as quoted data, the case as
facts, the corpus retrieved on the fragment; further turns and dives append to the thread;
every dive on the timeline), and the end-to-end journeys (all five green, with the
deep-scan job the planner runs over a claimed case: suppliers read for agreements, their
lists walked, contract findings raised, the plan on the timeline), and the delivery gate
script (one command over every invariant, every required suite and the canary, a red named
with its task and owner, a dated and sealed report with the fourteen-item manual checklist
appended) are in. The journeys suite (tests/e2e/journeys.test.ts) drives a real scan of
the estate through the front door, a fix applied by changing the fixture's responses, a
colleague's re-check, a sign-off, an export and a deletion, and CI runs every e2e suite on
push, files one at a time because the pg-boss queue is shared across test schemas; The
register page (draft rows from the scan, one form to confirm a row with its retention, the
record as a download, the count on the case page) closes journey 2; T-09 stays blocked on
A-06 and D-10 for journey 3 alone. The adversarial suite now covers seven injection
surfaces, cloaking (a browser against the declared scanner, CLK-01), exhaustion (loops,
stalls, huge pages, an archive bomb) and server-side request forgery (an egress guard in
the browser pool that judges every hop), against six hostile fixtures tagged
