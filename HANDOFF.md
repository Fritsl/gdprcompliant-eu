...
**35 of 112 done.** Phase 0 is complete and signed off; phase 1 is 10 of 11, phase 2 is
...
web shell, the fixture estate (with TLS), the browser pool, Pass A, cookie classification, the security
surface checks, policy discovery, the form inventory, replay and fingerprinting detection,
the database harness, the schema spine, the finding-needs-a-remedy constraint, row-level
security with a per-transaction tenant context, the durable job queue, the demand ledger and
the case object with numbering, tokens and claiming are in. The database is up
