...
**41 of 112 done.** Phase 0 is complete and signed off; phase 1 is 10 of 11, phase 2 is
14 of 34, phase 3 has begun and the task catalogue opens phase 5. Contracts, config with
the EU-only allowlist, the test harness, record and replay, i18n, the remedy catalogue and resolver, the model client, the
web shell, the fixture estate (with TLS), the browser pool, Pass A, cookie classification,
the security surface checks, policy discovery, the form inventory, replay and fingerprinting
detection, the database harness, the schema spine, the finding-needs-a-remedy constraint,
row-level security with a per-transaction tenant context, the durable job queue, the demand
...
and the typed task catalogue with its budgeted dispatcher are in. The database is up
(`pnpm db:up`); `T-07` is the last phase-1 task open.
