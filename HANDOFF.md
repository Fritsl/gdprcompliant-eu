...
**34 of 112 done.** Phase 0 is complete and signed off; phase 1 is 10 of 11 and phase 2 is
14 of 34. Contracts, config with the EU-only allowlist, the test harness, record and
...
security with a per-transaction tenant context, the durable job queue and the demand ledger
are in. The database is up (`pnpm db:up`); `T-07` is the last phase-1 task open.
