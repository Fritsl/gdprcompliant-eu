...
| `with_case` | tenants, case_events, evidence, findings, finding_evidence, vendors, processing_activities, answers, case_members, case_shares, mail_outbox, claim_verdicts, artefacts, graph_nodes, graph_edges | Go with the case: the owner's delete, or the unclaimed expiry below, removes them through the same `delete_case`. The tenant goes with its last case. |
